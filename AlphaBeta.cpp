...
int alphabeta(State st, int jugadorIA, int alpha, int beta, int profundidad, int profundidadMax)
{
        ...
        for (int y = 0; y < st.get_rows(); y++)
        {
            for (int x = 0; x < st.get_cols(); x++)
            {
                if (st.is_valid_move(x,y))
                {
                    ...
                    int valor =
                    alphabeta(hijo, jugadorIA, alpha, beta, profundidad + 1, profundidadMax);
...
std::pair<int,int> jugador_alphabeta(const State& st, int profundidadMax)
{
    int mejorValor = -1000;
    int mejorX = -1;
    int mejorY = -1;
    ...
    for (int y = 0; y < st.get_rows(); y++)
    {
        for (int x = 0; x < st.get_cols(); x++)
        {
            if (st.is_valid_move(x,y))
            {
                ...
                int valor =
                alphabeta(hijo, jugadorIA, -1000, 1000, 1, profundidadMax);
